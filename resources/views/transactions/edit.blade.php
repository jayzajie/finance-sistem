...
<div>
    ...
    <p class="page-subtitle">Halaman untuk mengedit transaksi</p>

    <div class="table-container">
        <div class="table-header">
            <h2 class="table-title">Form Edit Transaksi</h2>
            <a href="{{ $transaction->type == 'income' ? route('transactions.income') : route('transactions.expense') }}" class="btn btn-secondary">
        ...
        <div style="padding: 32px;">
            <form action="{{ route('transactions.update', $transaction) }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px;">
                    ...
                    <div>
                        ...
                        <select name="type" required 
                            ...
                            <option value="income" {{ old('type', $transaction->type) == 'income' ? 'selected' : '' }}>Kas Masuk (Pendapatan)</option>
                            <option value="expense" {{ old('type', $transaction->type) == 'expense' ? 'selected' : '' }}>Kas Keluar (Pengeluaran)</option>
                        </select>
                        @error('type')
                            <span style="color: #e53e3e; font-size: 12px;">{{ $message }}</span>
                        @enderror
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <textarea name="description" rows="3"
                            ...
                            onfocus="this.style.borderColor='#6366f1'; this.style.boxShadow='0 0 0 3px rgba(99, 102, 241, 0.1)'"
                            onblur="this.style.borderColor='#e2e8f0'; this.style.boxShadow='none'">{{ old('description', $transaction->description) }}</textarea>
                        @error('description')
                            <span style="color: #e53e3e; font-size: 12px;">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div style="margin-top: 32px; display: flex; gap: 12px; justify-content: flex-end;">
                    <a href="{{ $transaction->type == 'income' ? route('transactions.income') : route('transactions.expense') }}" class="btn btn-secondary">Batal</a>
